...
class AdminAuthRepo {
    ...
    async createAdmin(admin) {
        try {
            ...
            return AdminModel.create(freshAdmin)
        } catch (error) {
    ...
    async searchByEmailAndSelect(email, select) {
        try {
            return AdminModel.findOne({ email: email }).select(select)
        } catch (error) {
    ...
    async updateAdminDetails(userId, details) {
        try {
            ...
            return AdminModel.findOneAndUpdate({ _id: userId }, {...details})
        } catch (error) {
    ...
    async isUpdateEmailOccupied(id, email) {
        try {
            return AdminModel.findOne({_id: {$ne: id}, email: email })
        } catch (error) {
    ...
    async updateAdminPassword(email, password) {
        try {
            return AdminModel.findOneAndUpdate({ email: email }, { password: password })
        } catch (error) {
    ...
    async updateAdminImage(email, image) {
        try {
            return AdminModel.findOneAndUpdate({ email: email }, { image: image })
        } catch (error) {
    ...
    async updateAdminOTP(email, otp) {
        try {
            return AdminModel.findOneAndUpdate({ email: email }, { otp: otp })
        } catch (error) {
